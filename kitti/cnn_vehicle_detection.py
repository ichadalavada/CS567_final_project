...
def load_pkl_dataset(pkl_dir="kitti/kitti_pkl_224", use_merged=False, merged_percentage=None):
    pkl_dir = Path(pkl_dir)

    if use_merged and merged_percentage:
        # Load merged KITTI+IDD training data
        merged_train_path = Path("../merged_datasets") / f"kitti_idd_{merged_percentage}_train.pkl"
        print(f"Loading merged dataset: {merged_train_path}")
        
        with open(merged_train_path, "rb") as f:
            train_data = pickle.load(f)
        
        # Use validation data from KITTI for validation
        ...
        print(f"✓ Loaded KITTI val data: {len(val_data['images'])} images")
    else:
        # Load regular KITTI data
...
def main(use_merged=False, merged_percentage="5%", eval_on_idd=False):
    ...
    Args:
        use_merged: If True, train on merged KITTI+IDD dataset
    ...
    # ── Full evaluation on KITTI ───────────────────────────────────────
    y_pred, y_probs, acc_kitti = evaluate_model(
        model, X_val, y_val, metadata, 
        test_name="KITTI Validation"
    )
    ...
    if eval_on_idd:
        try:
            ...
            print(f"{'='*55}")
            print(f"  KITTI Validation Accuracy: {acc_kitti*100:.2f}%")
    ...
    # Save model
    if use_merged:
        model_path = f"mobilenet_kitti_idd_{merged_percentage}_classifier.keras"
    else:
        model_path = "mobilenet_kitti_classifier.keras"
