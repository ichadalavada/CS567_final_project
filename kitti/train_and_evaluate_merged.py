"""
Enhanced CNN training using merged KITTI + IDD datasets.
Trains on 1%, 5%, 10% IDD-augmented data and evaluates on both KITTI and IDD test sets.
...
def load_all_test_data():
    """Load KITTI validation (as test) and IDD test data"""
    kitti_test_path = Path("kitti_pkl_224/val.pkl")
    idd_test_path = Path("../IDD/idd_test.pkl")
    
    print("Loading test datasets...")
    kitti_test = load_pkl_dataset(str(kitti_test_path))
    ...
    return kitti_test, idd_test


def load_metadata():
    """Load KITTI metadata"""
    with open("kitti_pkl_224/metadata.pkl", "rb") as f:
...
def create_accuracy_matrix(results_df, save_path="accuracy_matrix.png"):
    ...
    plt.title("Model Accuracy Matrix\n(Trained on various KITTI+IDD mixes, tested on KITTI & IDD)")
...
def plot_accuracy_comparison(results_df, save_path="accuracy_comparison.png"):
    ...
    # KITTI results
    kitti_data = results_df[results_df["Test Data"] == "KITTI"]
    axes[0].bar(kitti_data["Training Data"], kitti_data["Accuracy (%)"], color="steelblue", alpha=0.7)
    axes[0].set_title("KITTI Test Set Performance", fontsize=12, fontweight="bold")
    axes[0].set_ylabel("Accuracy (%)")
    axes[0].set_ylim([0, 105])
    axes[0].grid(axis="y", alpha=0.3)
    for i, v in enumerate(kitti_data["Accuracy (%)"]):
...
def main():
    ...
    datasets_to_train = [
        ("1%", merged_dir / "kitti_idd_1%_train.pkl"),
        ("5%", merged_dir / "kitti_idd_5%_train.pkl"),
        ("10%", merged_dir / "kitti_idd_10%_train.pkl"),
    ]
    
    # Load test data
    kitti_test, idd_test = load_all_test_data()
    metadata = load_metadata()
    
    # Convert test data to classification format
    num_classes = metadata["num_classes"]
    X_kitti_test, y_kitti_test = convert_to_classification(kitti_test, num_classes=num_classes)
    X_kitti_test = X_kitti_test.astype("float32") / 255.0
    ...
    print(f"KITTI test samples: {len(y_kitti_test)}")
    if has_idd:
    ...
    for dataset_name, dataset_path in datasets_to_train:
        ...
        try:
            ...
            # Evaluate on KITTI test
            accuracy_kitti, y_pred_kitti, _, labels_kitti, names_kitti = evaluate_on_testset(
                model, X_kitti_test, y_kitti_test, 
                f"KITTI Test Set (n={len(y_kitti_test)})",
                metadata
            )
            
            results.append({
                "Training Data": f"KITTI + IDD ({dataset_name})",
                "Test Data": "KITTI",
                "Accuracy (%)": accuracy_kitti * 100,
                "Samples": len(y_kitti_test)
            })
            
            # Save KITTI confusion matrix
            plot_confusion_matrix(
                y_kitti_test, y_pred_kitti,
                f"KITTI Test - Trained on KITTI+IDD {dataset_name}",
                [list(metadata["classes"].values())[i] for i in labels_kitti],
                f"confusion_matrix_kitti_{dataset_name}.png"
            )
            ...
            if has_idd:
                try:
                    ...
                    plot_confusion_matrix(
                        ...
                        f"IDD Test - Trained on KITTI+IDD {dataset_name}",
            ...
            # Save model
            model_save_path = f"mobilenet_kitti_idd_{dataset_name}_classifier.keras"
    ...
    # Save CSV
    results_df.to_csv("accuracy_results_kitti.csv", index=False)
    print(f"\n✓ Results saved → accuracy_results_kitti.csv")
    ...
    print(f"{'='*80}")
    print(f"\nGenerated files:")
    print(f"  - accuracy_results_kitti.csv")
    print(f"  - accuracy_matrix.png")
    print(f"  - accuracy_comparison.png")
    print(f"  - confusion_matrix_kitti_*.png")
    if has_idd:
        print(f"  - confusion_matrix_idd_*.png")
    print(f"  - mobilenet_kitti_idd_*_classifier.keras")
